...
public struct Visualizer: View {
    ...
    public init(
        ...
        self.image = Self.loadDownsampled(uiImage)
        self.animationSpeed = animationSpeed
        self.blur = blur
        self.isStatic = isStatic
    }

    private static func loadDownsampled(_ uiImage: UIImage) -> Image {
        autoreleasepool {
    ...
    private var staticContent: some View {
        GeometryReader { geometry in
            ...
            layeredContent(size: size, time: 0)
        }
    }
    ...
    private var animatedContent: some View {
        TimelineView(.animation) { timeline in
            GeometryReader { geometry in
                ...
                layeredContent(size: size, time: time)
            }
        }
    }

    private func layeredContent(size: CGSize, time: Double) -> some View {
